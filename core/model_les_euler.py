...
class LES(object):
    ...
    def __init__(self, param, grid, linear=False):
        ...
        self.use_old_multigrid = False

        if self.use_old_multigrid:
            self.mg = mgorig.Multigrid(param, grid)
        else:
            npx = grid.npx
            npy = grid.npy
            nx = param["nx"]
            ...
            self.mg = MG(npx, npy, nx, ny, nz, nh, topology)

            self.mg.preallocate_for_nyles(
                grid.dx, param["neighbours"], self.halo)
    ...
    def diagnose_var(self, state):
        ...
        if self.use_old_multigrid:
            projection.compute_p_orig(
                self.mg, state, self.grid, self.neighbours)
        else:
